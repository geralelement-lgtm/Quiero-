...
// Paleta de colores unificada a tonos azules
const bluePalette = [
  new THREE.Color(0x0055ff), // Medium Blue
  new THREE.Color(0x00aaff), // Bright Blue
  new THREE.Color(0x0033cc), // Darker Blue
  new THREE.Color(0x0a0a20), // Very Dark Blue
...
const themes = {
  molten: {
    name: 'Ocean Blue',
    colors: bluePalette,
    bloom: { strength: 0.4, radius: 0.45, threshold: 0.7 }
  },
  cosmic: {
    name: 'Cosmic Blue',
    colors: bluePalette,
  ...
  emerald: {
    name: 'Electric Blue',
    colors: bluePalette,
    bloom: { strength: 0.4, radius: 0.6, threshold: 0.75 }
  }
};
...
function init() {
  ...
  const initialAnimationDuration = 8000; // AUMENTADO A 5 SEGUNDOS
  setTimeout(() => {
...
function getAttributesForParticle(i) {
  ...
  // Aceleración de color: 0.1
  const colorProgress = (t * colorPalette.length * 1.5 + time * 0.1) % colorPalette.length; 
...
function animateParticles() {
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    // Aceleración de ciclo de disolución: 1.2
    const cycleProgress = ((time * 1.2 + particleCycleOffset) % disintegrationCycleTime) / disintegrationCycleTime;
    ...
    // Factor LERP base (movimiento hacia el objetivo): 0.18
    let currentLerpFactor = 0.18; 
    ...
    if (disintegrationAmount > 0.001) {
      ...
      // Factor LERP en disolución: 0.1
      currentLerpFactor = 0.1 + disintegrationAmount * 0.03; 
    }
